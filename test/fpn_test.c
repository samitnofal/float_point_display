...
void test_validate_num_within_range()
{
    ...
    assert(validate_num_within_range(valid_number * 10) ==
           ERROR); // Overflow Case
    assert(validate_num_within_range(valid_number) == SUCCESS);
    assert(validate_num_within_range(valid_number_small) == SUCCESS);
}

char *SIMUTLATED_VALID_INPUT[5] = {"15.32", "14425.32", "2425", "0", "0.044"};
float SIMUTLATED_VALID_FLOAT[5] = {15.32, 14425.32, 2425, 0, 0.044};
char *SIMUTLATED_INVALID_INPUT[5] = {"15...", "-feafe", "00...03332", "0..0",
                                     ""};
...
char *read_input_stub_valid()
{
    ...
    assert(line != NULL); // No memory left
...
/*
    ...
    Runs get_float_point_number on different types of input and verifies that
   the function handles valid and invalid cases
*/
void test_get_float_point_number()
{
    ...
    for (idx = 0; idx < 5; idx++)
    {
        assert(get_float_point_number(&output_val, read_input_stub_valid) ==
               SUCCESS);
        assert(SIMUTLATED_VALID_FLOAT[idx] == output_val);
    }
    for (idx = 0; idx < 5; idx++)
    {
        assert(get_float_point_number(&output_val, read_input_stub_invalid) ==
               ERROR);
    }
}
...
int main()
{
    void (*test_func_p[NUMBER_OF_TESTS])() = {
        &test_validate_input_string,  &test_validate_num_within_range,
        &test_get_float_point_number, &test_print_float_bits,
        &test_print_header,
    };
